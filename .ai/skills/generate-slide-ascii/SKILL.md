...
- `{scenario}`: One of `zero-one`, `spaceship`, `fire`, or `null`
...
## Workflow
1. Read `{slide path}`
2. Validate `{scenario}` against `zero-one`, `spaceship`, `fire`, and `null`
...
```bash
pnpm ascii --slide "<slide path>" --scenario "<zero-one|spaceship|fire|null>"
```
