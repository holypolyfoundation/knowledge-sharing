...
- `{scenario}`: One of `zero-one`, `fire`, `radar`, `starfield`, `circuit-pulse`, `equalizer`, `packet-flow`, `terminal`, `game-of-life`, or `null`
...
## Workflow
1. Read `{slide path}`
2. Validate `{scenario}` against `zero-one`, `fire`, `radar`, `starfield`, `circuit-pulse`, `equalizer`, `packet-flow`, `terminal`, `game-of-life`, and `null`
...
```bash
pnpm ascii --slide "<slide path>" --scenario "<zero-one|fire|radar|starfield|circuit-pulse|equalizer|packet-flow|terminal|game-of-life|null>"
```
